// payment-service/src/outboxWorker.ts
...
export async function startOutboxWorker(logger: Logger): Promise<void> {
  const config = getCommonConfig("payment-service-outbox");
  const exchangeName = process.env.ORDER_EXCHANGE || "order.topic";

  logger.info("Starting Payment Service outbox worker", {
  ...
  const shutdown = async () => {
    logger.warn("Payment service outbox worker shutdown requested");
    await channel.close();
    ...
    try {
      ...
      for (const event of result.rows) {
        try {
          ...
          logger.info("Payment outbox event published", {
            eventId: event.id,
            type: event.type,
      ...
      logger.error("Payment outbox publish loop failed; retrying", {
        error: err.message,
      });
      shouldSleep = true;
    } finally {
