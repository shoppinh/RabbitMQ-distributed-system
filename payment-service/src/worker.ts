...
async function handleOrderCreated(
  ...
  const existing = await client.query(
    "SELECT id FROM payment_transactions WHERE saga_id = $1",
    [event.sagaId]
  );
  if ((existing.rowCount ?? 0) > 0) {
    logger.info("Payment saga already processed or cancelled, ignoring order created", {
      sagaId: event.sagaId,
    });
    return;
  }
  ...
  if (res.rows.length === 0) {
    ...
    const balance = Number.parseFloat(res.rows[0].balance);
    const amount = Number.parseFloat(String(event.payload["amount"]));
...
async function handleCancellationOrRefund(
  event: ConsumedEvent,
  client: PoolClient,
  logger: Logger
): Promise<void> {
  ...
  if ((paymentResult.rowCount ?? 0) === 0) {
    // Insert early cancel record so handleOrderCreated ignores it
    await client.query(
