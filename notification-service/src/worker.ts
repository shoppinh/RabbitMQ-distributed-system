...
async function sendOrderConfirmation(
  ...
  const message = `Order ${event.orderId} containing ${items?.length ?? 0} items for ${payload["amount"]} ${payload["currency"]} has been confirmed.`;
...
async function sendOrderCancellation(
  client: PoolClient,
  event: ConsumedEvent,
  logger: Logger
): Promise<void> {
